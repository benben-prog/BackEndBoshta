...
function getSslConfig() {
  ...
  if (env.NODE_ENV === "production") {
    // التحقق من وجود DATABASE_URL أولاً
    if (env.DATABASE_URL) {
      // إذا كان الاتصال محلياً أو لا يدعم SSL
      const isLocalhost = 
        env.DATABASE_URL.includes("localhost") ||
        env.DATABASE_URL.includes("127.0.0.1") ||
        env.DATABASE_URL.includes("31.97.193.19"); // خادمك البعيد
    ...
    const isLocalhost = 
      ...
      env.DB_HOST === "31.97.193.19"; // خادمك البعيد

    if (isLocalhost) {
      return false;
    }

    // للاتصالات عن بعد التي تدعم SSL
    return {
  ...
  // في بيئة التطوير - لا نستخدم SSL افتراضياً
...
function getPoolConfig() {
  const config = {
    connectionTimeoutMillis: parseInt(env.DB_POOL_CONNECTION_TIMEOUT) || 10000,
    idleTimeoutMillis: parseInt(env.DB_POOL_IDLE_TIMEOUT) || 10000,
